...
@torch.no_grad()
def encode_patch_tokens(images, model, tfm, device, batch_size, pool_grid):
    feats = []

    for start in tqdm(range(0, len(images), batch_size), desc="encoding DINOv2 patch tokens", leave=False):
        ...
        patches = out["x_norm_patchtokens"]  # [B, 256, 384] for 224x224 / patch14
        ...
        patches = patches.reshape(b, side, side, d)
        ...
        pooled = patches.reshape(
            ...
            block,
            d,
        ).mean(dim=(2, 4))  # [B, pool_grid, pool_grid, D]

        tokens = pooled.reshape(b, pool_grid * pool_grid, d)  # [B, 16, 384]

        feats.append(tokens.detach().cpu().numpy().astype(np.float16))

    return np.concatenate(feats, axis=0)


def build_transitions(tokens, poses, traj_dir, max_gap, hard_gap, mismatch_prob, rng):
    n = min(len(tokens), len(poses))
    tokens = tokens[:n]
    poses = poses[:n]
    ...
    return {
        "z_current": np.stack(z_current).astype(np.float16),
        "z_future": np.stack(z_future).astype(np.float16),
...
def main():
    ...
    merged["latent_shape"] = np.asarray(merged["z_current"].shape[1:])
    merged["latent_tokens"] = np.asarray(merged["z_current"].shape[1])
    merged["latent_dim"] = np.asarray(merged["z_current"].shape[2])
    merged["action_dim"] = np.asarray(merged["action"].shape[1])
    merged["source"] = np.asarray("tartanair_dinov2_patch_tokens_4x4")
    ...
    print(f"z_current shape={merged['z_current'].shape}")
    print(f"action_dim={merged['action'].shape[1]}")
