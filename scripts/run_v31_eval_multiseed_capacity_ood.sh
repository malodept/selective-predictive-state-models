...
mkdir -p logs/v31_multiseed_capacity_ladder_ood
mkdir -p reports/tables/protocol/pybullet_obstacle_rgb_encoder/v31_multiseed_capacity_ladder_ood
...
for MODEL in "${MODELS[@]}"; do
  ...
  for VARIANT in "${VARIANTS[@]}"; do
    ...
    OUT_DIR="reports/tables/protocol/pybullet_obstacle_rgb_encoder/v31_multiseed_capacity_ladder_ood/${VARIANT}"
    mkdir -p "$OUT_DIR"
    ...
    if [ ! -f "${OUT_DIR}/moving_only_${MODEL}.json" ]; then
      phipy -u scripts/eval_mixed_hard_tieaware_moving_only.py \
        ...
        --batch-groups 128 \
        --device cuda \
        2>&1 | tee "logs/v31_multiseed_capacity_ladder_ood/moving_only_${MODEL}_${VARIANT}.log"
    else
      echo "[SKIP] moving-only exists: ${OUT_DIR}/moving_only_${MODEL}.json"
    fi
    ...
    if [ ! -f "${OUT_DIR}/confidence_${MODEL}.json" ]; then
      phipy -u scripts/eval_state_disjoint_confidence_diagnostics.py \
        ...
        --batch-groups 128 \
        --device cuda \
        2>&1 | tee "logs/v31_multiseed_capacity_ladder_ood/confidence_${MODEL}_${VARIANT}.log"
    else
      echo "[SKIP] confidence exists: ${OUT_DIR}/confidence_${MODEL}.json"
    fi
  done
done

echo "DONE v31B multiseed capacity OOD eval"
