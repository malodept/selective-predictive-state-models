...
run_variant () {
  ...
  echo "======================================================================"
  echo "v33D variant=${NAME} blocked=${BLOCKED} horizon=${HORIZON} velocity=${VELOCITY} seed=${SEED}"
  echo "======================================================================"
...
for BLOCKED in 1 2 3 4; do
  for HORIZON in 36 72; do
    for SEED in 20 21 22; do
      NAME="block${BLOCKED}_h${HORIZON}_seed${SEED}"
      run_variant "$NAME" "$BLOCKED" "$HORIZON" "1.2" "$SEED"
    done
  done
done

echo "DONE v33D full OOD mini-grid"
